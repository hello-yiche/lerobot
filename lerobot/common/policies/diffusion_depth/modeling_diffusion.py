...
class DiffusionPolicy(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        self.expected_image_keys = [
            k for k in config.input_shapes if (k.startswith("observation.image") and not ("depth" in k))]
        print("EXPECTED IMAGE KEYS: ", self.expected_image_keys)

        self.expected_depth_keys = [
            k for k in config.input_shapes if (k.startswith("observation.image") and ("depth" in k))]
        print("EXPECTED DEPTH KEYS: ", self.expected_depth_keys)
    ...
    def reset(self):
        ...
        self._queues = {
            ...
            "observation.depths": deque(maxlen=self.config.n_obs_steps),
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        batch["observation.depths"] = torch.stack(
            [batch[k] for k in self.expected_depth_keys], dim=-4)
...
class DiffusionModel(nn.Module):
    def __init__(self, config: DiffusionConfig):
        ...
        num_depth_images = len(
            [k for k in config.input_shapes if (k.startswith("observation.image") and "depth" in k)])
        num_rgb_images = num_images - num_depth_images

        self.unet = DiffusionConditionalUnet1d(
            config,
            global_cond_dim=(
                config.input_shapes["observation.state"][0] +
                self.rgb_encoder.feature_dim * num_rgb_images +
                self.depth_encoder.feature_dim * num_depth_images
            )
            * config.n_obs_steps,
        )
    ...
    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> Tensor:
        """Encode image and depth features and concatenate them all together along with the state vector."""
        batch_size, n_obs_steps = batch["observation.state"].shape[:2]
        # Extract image feature (first combine batch, sequence, and camera index dims).
        img_features = self.rgb_encoder(
            einops.rearrange(batch["observation.images"],
                             "b s n ... -> (b s n) ...")
        )
        ...
        depth_features = self.depth_encoder(
            einops.rearrange(batch["observation.depths"],
                             "b s n ... -> (b s n) ...")
        )
        ...
        return torch.cat([batch["observation.state"], img_features, depth_features], dim=-1).flatten(start_dim=1)
    ...
    def generate_actions(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        This function expects `batch` to have:
        {
            ...
            "observation.depths": (B, n_obs_steps, num_cameras, C, H, W)
        }
        """
        batch_size, n_obs_steps = batch["observation.state"].shape[:2]
    ...
    def compute_loss(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        assert set(batch).issuperset(
            {"observation.state", "observation.images", "observation.depths", "action", "action_is_pad"})
...
class DiffusionRgbEncoder(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig):
        ...
        image_keys = [k for k in config.input_shapes if k.startswith(
            "observation.image")]
        # Note: we have a check in the config class to make sure all images have the same shape.
        image_key = image_keys[0]
        dummy_input_h_w = (
            config.crop_shape if config.crop_shape is not None else config.input_shapes[
                image_key][1:]
        )
...
class DiffusionDepthEncoder(nn.Module):
    """Encoder a depth image into a 1D feature vector.
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Args:
            x: (B, C, H, W) depth tensor with pixel values in [0, 1].
        Returns:
            (B, D) depth feature.
        """
        # Preprocess: maybe crop (if it was set up in the __init__).
...
class DiffusionConditionalUnet1d(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig, global_cond_dim: int):
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.Conv1d(config.down_dims[0],
                      config.output_shapes["action"][0], 1),
        )
